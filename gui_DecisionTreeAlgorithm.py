...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        # Predictions
        predictions6 = regg.predict(newArrayNorm)
        print(predictions6[0])
        MainPredictedDifference1 = float (predictions6[0]) - float (readedValuesfromPLC[2])
        ...
        Vol_Data7.append(float(predictions6[0]))
        ...
        print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_Data7))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV7', [XdataGraphs, Vol_Data7])
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        Vol_Data7 = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_Data7, label="Decision Tree", parent="y_axisV", tag="rev_plotV7")
...
regg = DecisionTreeRegressor(max_depth=30).fit(NewArrayList, train_labels.values.ravel())
