...
# Start Part of Max
data_set = pd.read_csv("Data1710.txt",
					delimiter = ';',low_memory=False, encoding='latin-1')

data_set.columns = ["RotationMotorA", 'Volumenstrom', 'Regelventil', 'Motor_A_Elektrische_Leistung']

# store dataframe into csv file
data_set.to_csv('Data for Learning.csv',
			index = None)

columns = ["RotationMotorA", 'Volumenstrom', 'Regelventil', 'Motor_A_Elektrische_Leistung']

data_set = data_set.loc[:, columns]
fullstand2 = ['Volumenstrom']
#featuresdrop = ['Volumenstrom', "Temp_V1", "Motor_A_Wirkleistung", "Motor_A_Leistungsfaktor"]

feature1 = ["RotationMotorA"]
data_set1 = data_set[feature1]

feature2 = ['Volumenstrom']
data_set2 = data_set[feature2]

feature3 = ['Motor_A_Elektrische_Leistung']
data_set3 = data_set[feature3]
...
global features
global labels
features = data_set.drop(fullstand2, axis=1)
...
EngineA_Power               = cp(Plc, "Engine A Power",                 dt.Real,    268,    0,  27)
EngineA_Wirkleistung               = cp(Plc, "EngineA_Wirkleistung",                dt.Real,    64,    0,  15)
EngineA_Leistungsfaktor               = cp(Plc, "EngineA_Leistungsfaktor",          dt.Real,    72,    0,  15)
Temp               = cp(Plc, "Temp",          dt.Real,    540,    0,  33)
...
A2                          = cp(Plc, "A2",                             dt.Real,    520,    0,  33)
B1                          = cp(Plc, "B1",                             dt.Real,    524,    0,  33)
R1                          = cp(Plc, "R1",                             dt.Real,    532,    0,  33)
R2                          = cp(Plc, "R2",                             dt.Real,    536,    0,  33)
...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        readedValuesfromPLC = [MotorA_Drehzahl.Value, Temp.Value, Volumenstrom.Value,
                               Ventil.Value, EngineA_Power.Value,
                               EngineA_Wirkleistung.Value, EngineA_Leistungsfaktor.Value]
        ...
        newArray = (MotorA_Drehzahl.Value,
                    Ventil.Value, EngineA_Power.Value)


        FirstArray0 = (newArray[0] - DrehZahlMin) / (DrehZahlMax - DrehZahlMin)
        SecondArray0 = (newArray[1] - VentMin) / (VentMax - VentMin)
        ThirdArray0 = (newArray[2] - ElekLeistMin) / (ElekLeistMax - ElekLeistMin)

        newArrayNorm = np.array ([[FirstArray0, SecondArray0, ThirdArray0], [FirstArray0, SecondArray0, ThirdArray0]]).reshape(-1, 1, 3)
        print(newArrayNorm)
        # Predictions

        volumData = (readedValuesfromPLC[2])
        print ("Volumenstrom", readedValuesfromPLC[2])
        #array3 = np.asarray(volumData)
        #array3 = array3.reshape(1, -1)
        #Scale_array3 = scale(array3)
        #Scale_array3 = abs (Scale_array3)
        ...
        #Scale_array2 = scale(newArray)
        #print ("Scaled Data", Scale_array2)
        #print (Scale_array2)
        #array2 = np.asarray(Scale_array2)
        #array2 = array2.reshape(-1, 1, 3)
        #print(array2)

        # Predictions
        predictions9 = model.predict(newArrayNorm)
        predictions9 = abs (predictions9)
        print ("Data from predictions", predictions9)
        MainPredictedDifference1 = float (predictions9[0]) - float (readedValuesfromPLC[2])
        #MainPredictedDifference2 = float(predictions5) - float(readedValuesfromPLC[2])
        #MainPredictedDifference3 = float(predictions7) - float(readedValuesfromPLC[2])
        #MainPredictedDifference4 = float(predictions8) - float(readedValuesfromPLC[2])
        #MainPredictedDifference5 = float(predictions9) - float(readedValuesfromPLC[2])

        #AllDifferences = [MainPredictedDifference1, MainPredictedDifference2, MainPredictedDifference3,
        #                  MainPredictedDifference4, MainPredictedDifference5]
        #print (AllDifferences)

        MeanValue.append(float(abs(readedValuesfromPLC[2])))
        MeanValueDiffer = mean (MeanValue)
        print("MEAN Average REAL value of the list:")
        print(MeanValueDiffer)

        #AllDifferencesMin = min (AllDifferences)
        #print (AllDifferencesMin)

        #FinalIndex = AllDifferences.index(AllDifferencesMin)
        #print (FinalIndex)

        global i
        XdataGraphs = list(np.arange(0, i, 0.5))

        Vol_Data.append(float(readedValuesfromPLC[2]))
        Vol_Data10.append(float(predictions9[0]))


        A1_DataG.append(float(readedValuesfromPLC[5]))
        B1_DataG.append(float(readedValuesfromPLC[1]))
        R1_DataG.append(float(readedValuesfromPLC[4]))
        VEN_DataG.append(float(readedValuesfromPLC[3]))
        MotA_DataG.append(float(readedValuesfromPLC[4]))
        N_DataA.append(float(readedValuesfromPLC[0]))

        Differenz.append(float(abs(MainPredictedDifference1)))
        MeanDifferenz = mean (Differenz)
        print("Average value of the list:")
        print (MeanDifferenz)

        #print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_Data10))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV10', [XdataGraphs, Vol_Data10])
...
def ComponentReadData():
    if(A1 != None):
        ...
        readedValuesfromPLC = [A1.Value, B1.Value, R1.Value, Volumenstrom.Value,
                               Ventil.Value, EngineA_Power.Value, EngineB_Power.Value, MotorA_Drehzahl.Value, MotorB_Drehzahl.Value, A2.Value, R2.Value]
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        dpg.set_axis_limits("y_axisV",0,33)
        dpg.set_axis_limits("x_axisV", 0, i+5)

        global Vol_Data
        Vol_Data = [0, readedValuesfromPLC[2]]
        Vol_Data10 = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_Data10, label="Keras Model", parent="y_axisV", tag="rev_plotV10")
...
with dpg.window(label="Rotational speed", pos=(600, 265)) as graphs7:
    ...
    with dpg.plot(height=220, width=430):
        ...
        global N_DataA
        #global N_DataB
        N_DataA = [0, readedValuesfromPLC[5]]
...
h2 = 2
NewArrayList = []
for i0 in train_features.values:
    train_features.values[g][h1]
    FirstArray1 = (train_features.values[g][h0] - DrehZahlMin) / (DrehZahlMax - DrehZahlMin)
    SecondArray1 = (train_features.values[g][h1] - VentMin) / (VentMax - VentMin)
    ThirdArray1 = (train_features.values[g][h2] - ElekLeistMin) / (ElekLeistMax - ElekLeistMin)
    g += 1
    NewArrayData = [FirstArray1, SecondArray1, ThirdArray1]
    NewArrayList.append(NewArrayData)
...
Keras_trained = np.asarray (NewArrayList)
Keras_trained = Keras_trained.reshape(-1, 1, 3)

Keras_y_train = np.asarray (train_labels)
Keras_y_train = Keras_y_train.reshape(-1, 1, 1)

inputs = keras.Input(shape=(1, 3))

x = layers.LSTM(128, activation="relu", name="dense_1")(inputs)
x = layers.Dropout(0.3, noise_shape=None, seed=None)(x)
...
model.fit(Keras_trained, Keras_y_train, epochs=10, batch_size=1)
model.save('LSTMmodel1710.h5')
print ("Keras DONE")
