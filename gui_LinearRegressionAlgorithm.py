...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        # Predictions
        predictions5 = lin_reg_pc.predict(newArrayNorm)
        print(predictions5[0])

        MainPredictedDifference1 = float (predictions5[0]) - float (readedValuesfromPLC[2])
        ...
        Vol_Data6.append(float(predictions5[0]))
        ...
        print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_Data6))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV6', [XdataGraphs, Vol_Data6])
        ...
        dpg.set_axis_limits("y_axisMotorA", 0, 3)
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        Vol_Data6 = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_Data6, label="Elastic Regression", parent="y_axisV", tag="rev_plotV6")
...
lin_reg_pc = LinearRegression().fit(NewArrayList, train_labels.values.ravel())
