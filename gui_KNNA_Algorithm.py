...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        print(newArrayNorm)
        # Predictions
        predictions7 = knn.predict(newArrayNorm)
        print(predictions7[0])

        MainPredictedDifference1 = float (predictions7[0]) - float (readedValuesfromPLC[2])
        ...
        Vol_Data8.append(float(predictions7[0]))
        ...
        print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_Data8))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV8', [XdataGraphs, Vol_Data8])
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        Vol_Data8 = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_Data8, label="KNN Regression", parent="y_axisV", tag="rev_plotV8")
...
knn = KNeighborsRegressor(n_neighbors=200, weights = 'distance').fit(NewArrayList, train_labels.values.ravel())
print ("KNNA DONE")
