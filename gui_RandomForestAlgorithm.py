...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        newArrayNorm = np.array([[FirstArray0, SecondArray0, ThirdArray0], [FirstArray0, SecondArray0, ThirdArray0]])
        print(newArrayNorm)
        ...
        #Scale_array2 = scale(array1)
        #array2 = np.asarray(newArray)
        #array2 = array2.reshape(-1, 1, 3)
        #print(array2)

        # Predictions
        predictions8 = randomForest.predict(newArrayNorm)
        print(predictions8[0])
        MainPredictedDifference1 = float (predictions8[0]) - float (readedValuesfromPLC[2])
        ...
        Vol_Data9.append(float(predictions8[0]))
        ...
        print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_Data9))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV9', [XdataGraphs, Vol_Data9])
        ...
        dpg.set_axis_limits("y_axisMotorA", 0, 3.5)
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        Vol_Data9 = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_Data9, label="Random Forest", parent="y_axisV", tag="rev_plotV9")
...
with dpg.window(label="Absolute Error between Real and Predicted Flow ", pos=(10, 530)) as graphs8:
    ...
    with dpg.plot(height=220, width=565):
        ...
        dpg.set_axis_limits("y_axisDiff", 0, 20)
        dpg.set_axis_limits("x_axisDiff", 0, i + 5)
...
h2 = 2
NewArrayListTest = []
for i1 in test_features.values:
    test_features.values[g1][h1]
    FirstArray2 = (test_features.values[g1][h0] - DrehZahlMin) / (DrehZahlMax - DrehZahlMin)
    SecondArray2 = (test_features.values[g1][h1] - VentMin) / (VentMax - VentMin)
    ThirdArray2 = (test_features.values[g1][h2] - ElekLeistMin) / (ElekLeistMax - ElekLeistMin)
    g1 += 1
    NewArrayData2 = [FirstArray2, SecondArray2, ThirdArray2]
    NewArrayListTest.append(NewArrayData2)


global randomForest

randomForest = RandomForestRegressor(n_estimators = 100, random_state = 0).fit(NewArrayList, train_labels.values.ravel())
