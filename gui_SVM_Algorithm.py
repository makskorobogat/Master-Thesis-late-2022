...
def Onactualisationgrafics():
    ...
    while stop == False:
        ...
        print(newArrayNorm)
        # Predictions
        predictions1 = svr_regressor.predict(newArrayNorm)
        print (predictions1[0])

        MainDifference = float (predictions1[0]) - float (readedValuesfromPLC[2])
        ...
        Vol_DataPred.append(predictions1[0])
        ...
        print('How accurate it predicts: %.3f' % r2_score(Vol_Data, Vol_DataPred))

        # Set part for Vol
        dpg.set_value('rev_plotV', [XdataGraphs, Vol_Data])
        dpg.set_axis_limits("x_axisV", 0, i + 5)
        dpg.set_value('rev_plotV11', [XdataGraphs, Vol_DataPred])
...
with dpg.window(label="Volumenstrom", pos=(10, 265)) as graphs3:
    ...
    with dpg.plot(height=210, width=565):
        ...
        Vol_DataPred = [0, readedValuesfromPLC[2]]

        # series belong to a y axis
        dpg.add_line_series(XdataGraphs, Vol_Data, label="Real Flow", parent="y_axisV", tag="rev_plotV")
        dpg.add_line_series(XdataGraphs, Vol_DataPred, label="SVR Regression", parent="y_axisV", tag="rev_plotV11")
...
svr_regressor = NuSVR(cache_size=1000).fit(NewArrayList, train_labels.values.ravel())
print ("SVR DONE")
